...
document.getElementById("fecha_mop").value = fechaISO;
...
function actualizarContextoFormulario() {
  ...
  const selectionMap = {
    "formulario_mop_programacion_responsive.html?origen=fallo&tipo=mop": {
      title: "MOP de Fallo",
      subtitle: "Procedimiento de Operación",
    },
    "formulario_mop_programacion_responsive.html?origen=instalacion&tipo=mop": {
      title: "MOP de Instalación",
      subtitle: "Procedimiento de Operación",
    },
    "formulario_mop_programacion_responsive.html?origen=programacion-mantenimiento&tipo=mop":
      {
        title: "MOP de Mantenimiento",
        subtitle: "Procedimiento de Operación",
      },
    "formulario_mop_programacion_responsive.html?origen=programacion-correctiva&tipo=mop":
      {
        title: "MOP de Programación Correctiva",
        subtitle: "Procedimiento de Operación",
      },
    "formulario_mop_programacion_responsive.html": {
      title: "MOP Programación",
      subtitle: "Procedimiento de Operación",
    },
  };
  ...
  const selection =
    ...
    selectionMap["formulario_mop_programacion_responsive.html"];
...
function guardarDatosFormulario() {
  ...
  localStorage.setItem("mopFormDraftProg", JSON.stringify(data));
}

function cargarDatosFormulario() {
  const savedData = localStorage.getItem("mopFormDraftProg");
  if (!savedData) return;

  try {
    const data = JSON.parse(savedData);
    for (let [key, value] of Object.entries(data)) {
      if (key === "fecha_mop") continue;
...
document.getElementById("mopForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const response = await fetch(
      "https://form.gererlenergie.com/api/informes/crearProgramacion",
      {
        method: "POST",
        body: formData,
        credentials: "include",
    ...
    if (response.ok) {
      ...
      downloadLink.href = `https://form.gererlenergie.com/api/mops-programacion/descargar/${data.informeId}`;
      const origen = new URLSearchParams(window.location.search).get('origen') || '';
      const prefijosMap = {
        'instalacion': 'MOP_Instalacion',
        'programacion-correctiva': 'MOP_Programacion_Correctiva',
        'programacion-mantenimiento': 'MOP_Mantenimiento',
      };
      const prefijo = prefijosMap[origen] || 'MOP_Programacion';
      const titulo = document.querySelector('[name="titulo_procedimiento_programacion"]')?.value || 'Sin_titulo';
      ...
      localStorage.removeItem("mopFormDraftProg");
      btn.innerHTML =
