...
// Endpoint para validar sesión activa desde frontend
...
// ================================
// ENDPOINTS MOPS MANTENIMIENTO PREVENTIVO
// ================================
app.get("/api/mops-mantenimiento", authMiddleware, requireRol("admin"), async (req, res) => {
  try {
    const db = require("./database/db_connect");
    const page = Math.max(1, parseInt(req.query.page) || 1);
    const limit = Math.min(50, Math.max(1, parseInt(req.query.limit) || 15));
    ...
    const [countResult] = await db.query("SELECT COUNT(*) as total FROM mops_mantenimiento");
    const total = countResult[0].total;
    const totalPages = Math.ceil(total / limit);

    const [mops] = await db.query(`
      SELECT m.id, m.nombre_archivo, m.created_at, u.nombre as usuario_nombre, u.email as usuario_email
      FROM mops_mantenimiento m
      INNER JOIN usuarios u ON m.usuario_id = u.id
      ORDER BY m.created_at DESC
      LIMIT ? OFFSET ?
    `, [limit, offset]);
    ...
    console.error("Error listando MOPs Mantenimiento:", error);
    res.status(500).json({ error: "Error al listar MOPs", details: error.message });
  }
});

app.post("/api/mops-mantenimiento/buscar", authMiddleware, requireRol("admin"), async (req, res) => {
  try {
    const db = require("./database/db_connect");
    const rawQuery = String(req.body?.q || req.query.q || '').trim();
    if (!rawQuery) return res.status(400).json({ error: "Se requiere término de búsqueda" });
    ...
    const [countResult] = await db.query(
      "SELECT COUNT(*) as total FROM mops_mantenimiento m INNER JOIN usuarios u ON m.usuario_id = u.id WHERE m.nombre_archivo LIKE ? OR u.nombre LIKE ?",
      [searchTerm, searchTerm]
    );
    const total = countResult[0].total;
    const totalPages = Math.ceil(total / limit);
    ...
    console.error("Error buscando MOPs Mantenimiento:", error);
    res.status(500).json({ error: "Error al buscar", details: error.message });
  }
});

app.delete("/api/mops-mantenimiento/:id", authMiddleware, requireRol("admin"), async (req, res) => {
  try {
    const db = require("./database/db_connect");
    const [result] = await db.query("DELETE FROM mops_mantenimiento WHERE id = ?", [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ error: "MOP no encontrado" });
    res.json({ message: "MOP eliminado exitosamente" });
  } catch (error) {
    console.error("Error eliminando MOP Mantenimiento:", error);
    res.status(500).json({ error: "Error al eliminar", details: error.message });
  }
});
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log('📋 num_ticket:', req.body.num_ticket);
      console.log('📋 num_ot:', req.body.num_ot);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log('📋 req.body keys (renov):', Object.keys(req.body));
      console.log('📋 num_ticket (renov):', req.body.num_ticket);
      console.log('📋 num_ot (renov):', req.body.num_ot);
...
// Endpoint generar informe de mantenimiento preventivo
const {
  generarInformeMantenimiento,
} = require("./controllers/documentMantenimientoController");
app.post(
  "/api/informes/crearMantenimiento",
  authMiddleware,
  createLimiter,
  upload.fields([
    { name: "firma_m", maxCount: 1 },
    { name: "foto_frecuencia_m", maxCount: 1 },
    { name: "foto_nivel_carga_m", maxCount: 1 },
    { name: "foto_estado_baterias_m", maxCount: 1 },
  ]),
  async (req, res) => {
    try {
      req.savedImages = {};
      ...
      console.log('📋 req.body keys (mant):', Object.keys(req.body));

      generarInformeMantenimiento(req, res);
    } catch (err) {
      console.error("Error procesando solicitud:", err.message);
      return res
        .status(500)
...
// Endpoint descargar MOP Mantenimiento
app.get(
  "/api/mops-mantenimiento/descargar/:informeId",
  authMiddleware,
  async (req, res) => {
    try {
      const db = require("./database/db_connect");
      ...
      let query = "SELECT nombre_archivo, archivo FROM mops_mantenimiento WHERE id = ?";
      const params = [informeId];
      ...
      console.error("Error descargando MOP Mantenimiento:", error);
      res
        .status(500)
        .json({ error: "Error al descargar archivo", details: error.message });
    }
...
app.post(
  ...
  upload.fields([
    ...
    { name: "firma_elaborador", maxCount: 1 },
  ]),
  async (req, res) => {
    try {
      // Convertir req.files a req.savedImages
