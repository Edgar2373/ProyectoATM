...
const generarInformeRenovacion = async (req, res) => {
  try {
    ...
    const tituloMop = (req.body.titulo_mop_r || 'Sin_titulo').replace(/\s+/g, '_').replace(/[^\w\-_áéíóúñüÁÉÍÓÚÑÜ]/g, '');
    const fechaMop = (req.body.fecha_mop_r || '').replace(/-/g, '-');
    const tipoMapRen = { 'fallo': 'MOP_Fallo', 'cambio-bateria': 'MOP_Cambio_Bateria' };
    const tipoRen = tipoMapRen[req.body.origen] || 'MOP_Renovacion';
    const nombreArchivo = `${tituloMop}_${fechaMop}_${tipoRen}.docx`;
