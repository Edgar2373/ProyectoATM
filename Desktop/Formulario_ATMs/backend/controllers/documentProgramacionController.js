...
const generarInformeProgramacion = async (req, res) => {
  try {
    ...
    const ImageOpts = {
      ...
      getSize: (img, tagValue, tagName) => {
        try {
          ...
          // Mantener proporción y no agrandar imágenes pequeñas.
    ...
    const tituloMop = (req.body.titulo_procedimiento_programacion || 'Sin_titulo').replace(/\s+/g, '_').replace(/[^\w\-_áéíóúñüÁÉÍÓÚÑÜ]/g, '');
    const fechaMop = (req.body.fecha_mop || '').replace(/-/g, '-');
    const tipoMapProg = { 'instalacion': 'MOP_Instalacion', 'programacion-correctiva': 'MOP_Programacion_Correctiva', 'programacion-mantenimiento': 'MOP_Mantenimiento' };
    const tipoProg = tipoMapProg[req.body.origen] || 'MOP_Programacion';
    const nombreArchivo = `${tituloMop}_${fechaMop}_${tipoProg}.docx`
